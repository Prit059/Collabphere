import React, { useState, useEffect } from "react";
  ...
  useEffect(() => {
    ...
    const handleRemoveClub = async () => {
      try {
        const response = await fetch(`http://localhost:5000/remove-club/${clubToRemove}`, {
          method: "DELETE",
        });
        ...
        alert("Failed to remove club. Please try again.");
      }
    };
      ...
      <section className="container-fluid highlights py-5 sec1" id="club-section">
        <div className="container">
          ...
          {isLoggedIn && showModal && (
            <div className="club-form-modal">
              <div className="modal-content">
                <h3 className="form-title">Add a New Club</h3>
                <form className="club-form">
                  <div className="form-group">
                    ...
                    <input 
                      ...
                      value={newClub.faclty} 
                      onChange={(e) => setNewClub({ ...newClub, faclty: e.target.value })} 
                    />
                    <input 
                      type="text" 
                      placeholder="Student Coordinator Name" 
                      value={newClub.student} 
                      onChange={(e) => setNewClub({ ...newClub, student: e.target.value })} 
                    />
                    <input 
                      type="file" 
                      accept="image/*" 
                    ...
                    />
                    {newClub.image && (
                      <img src={newClub.image} alt="Preview" className="preview-img" />
                    )}
                  </div>
                  <div className="button-container">
                    <button type="button" className="submit-btn" onClick={handleAddClub} id="clubsubmit">
                      Submit
                    </button>
                    <button type="button" className="cancel-btn" onClick={() => setShowModal(false)} id="clubcancel">
                      Cancel
                    </button>
                  </div>
                </form>
          ...
          {showConfirm && (
            <div className="club-form-modal-con">
              <div className="modal-content-con ">
                <h3 className="form-title-confirm">Are you sure you want to remove this club?</h3>
                <div className="button-container-con">
                  <button className="submit-btn-con" onClick={handleRemoveClub}>
                    Yes, Remove
                  </button>
                  <button className="cancel-btn-con" onClick={() => setShowConfirm(false)}>
                    Cancel
                  </button>
                </div>
              </div>
          ...
          {isLoggedIn && showForm && (
            <div className="club-form-modal-join">
              <div className="modal-content-join">
                <h3 className="form-title">Join Club</h3>
                <form className="club-form">
